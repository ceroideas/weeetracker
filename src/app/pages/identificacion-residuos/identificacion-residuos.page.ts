...
export class IdentificacionResiduosPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.myForm.valueChanges.subscribe(form => {
      if (this.myForm.get("albaran").status == "VALID" && (this.myForm.get("centro").status == "VALID" || this.myForm.get("centro").status == "DISABLED") && this.myForm.get("nombre").status == "VALID") 
      {
